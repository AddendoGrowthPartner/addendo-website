...
const steps = [
  ...
  { num: '02', title: 'Disenamos tu estrategia', text: 'Plan de crecimiento a tu medida. Cada accion con proposito, cada dolar con destino claro.' },
  { num: '03', title: 'Ejecutamos sin parar', text: 'Sitio web, campanas, contenido, seguimiento de leads — todo coordinado, todo funcionando.' },
  ...
  useEffect(() => {
    import('gsap').then(({ gsap }) => {
      import('gsap/ScrollTrigger').then(({ ScrollTrigger }) => {
        ...
        if (!el) return;
        gsap.fromTo(el.querySelector('.pr-title'), { opacity: 0, y: 50 }, { opacity: 1, y: 0, duration: 0.8, scrollTrigger: { trigger: el, start: 'top 80%' } });
        el.querySelectorAll('.pr-step').forEach((s, i) => {
          gsap.fromTo(s, { opacity: 0, x: i % 2 === 0 ? -40 : 40 }, { opacity: 1, x: 0, duration: 0.7, scrollTrigger: { trigger: s, start: 'top 85%' } });
        });
      });
    });
  }, []);
  ...
  return (
    <section ref={ref} id="proceso" className="section-pad relative" style={{ background: '#0A1228' }}>
      <div className="max-w-5xl mx-auto">
        <h2 className="pr-title opacity-0 font-display text-3xl sm:text-4xl md:text-5xl font-bold text-center mb-20">
          De donde estas a donde quieres llegar.<br /><span className="gold-gradient-text">En 4 pasos.</span>
        </h2>
        ...
        <div className="relative">
          ...
          <div className="space-y-16 md:space-y-24">
            {steps.map((s, i) => (
              <div key={i} className={`pr-step opacity-0 relative flex flex-col md:flex-row gap-6 md:gap-14 ${i % 2 === 1 ? 'md:flex-row-reverse' : ''}`}>
