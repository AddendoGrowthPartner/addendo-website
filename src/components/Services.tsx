  ...
  useEffect(() => {
    import('gsap').then(({ gsap }) => {
      import('gsap/ScrollTrigger').then(({ ScrollTrigger }) => {
        ...
        gsap.fromTo(el.querySelector('.services-title'), { opacity: 0, y: 50 },
        ...
        el.querySelectorAll('.service-card').forEach((card, i) => {
          gsap.fromTo(card, { opacity: 0, y: 40, scale: 0.95 },
            { opacity: 1, y: 0, scale: 1, duration: 0.6, delay: i * 0.08, scrollTrigger: { trigger: card, start: 'top 92%' } });
        });
      });
    });
  }, []);
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
          {services.map((service, i) => (
            <div key={i} className={`service-card opacity-0 glass-card p-7 group hover:border-gold/40 transition-all duration-500 hover:-translate-y-1 ${i === 6 ? 'md:col-span-2 lg:col-span-1 lg:col-start-2' : ''}`}>
              <div className="text-gold mb-4 group-hover:scale-110 transition-transform duration-300">{service.icon}</div>
              <h3 className="text-lg font-bold text-white mb-1">{service.title}</h3>
              <p className="text-gold/80 text-sm font-medium mb-3">{service.subtitle}</p>
              <p className="text-white/50 leading-relaxed mb-5 text-sm">{service.text}</p>
